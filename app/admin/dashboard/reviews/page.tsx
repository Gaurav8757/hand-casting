...
export default function ReviewsManagement() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editingId) {
        ...
        const { error } = await supabase.from("customer_ratings").insert(payload)
  ...
  async function handleDeleteConfirm() {
    ...
    try {
      const { error } = await supabase.from("customer_ratings").delete().eq("id", deleteId)
      if (error) throw error
  ...
  async function handleBulkDelete() {
    ...
    try {
      const { error } = await supabase.from("customer_ratings").delete().in("id", selectedIds)
      if (error) throw error
  ...
  async function toggleDisplayOnLanding(id: string, currentStatus: boolean) {
    try {
      const { error } = await supabase
        .from("customer_ratings")
        .update({ display_on_landing: !currentStatus, updated_at: new Date().toISOString() })
        .eq("id", id)
  ...
  return (
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 px-2">
        <div>
          <h2 className="text-4xl font-extrabold text-foreground tracking-tighter uppercase italic underline decoration-amber-500/40 underline-offset-8">Echoes of Delight</h2>
          <p className="text-foreground/60 mt-2 text-lg font-medium">Manage the voice of your satisfied clientele.</p>
        </div>
        <div className="flex items-center gap-3">
           <div className="bg-white/5 border border-white/10 px-5 py-3 rounded-2xl flex items-center gap-3 shadow-xl">
              <MessageSquare className="w-5 h-5 text-amber-500" />
              <span className="font-bold text-foreground">{reviews.length} Testimonials</span>
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
        ...
        <div className="xl:col-span-4">
          <div className="glass p-8 sticky top-24 border-white/20 shadow-4xl relative overflow-hidden group">
            ...
            <h3 className="text-2xl font-black text-foreground mb-8 flex items-center gap-3 relative z-10">
              {editingId ? (
                <div className="p-2.5 bg-amber-500/20 rounded-2xl shadow-inner"><Edit2 className="w-5 h-5 text-amber-500" /></div>
              ) : (
                <div className="p-2.5 bg-amber-500/20 rounded-2xl shadow-inner"><Plus className="w-5 h-5 text-amber-500" /></div>
              )}
            ...
            <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
              <div className="space-y-1.5">
                <label className="text-xs font-black uppercase tracking-[0.2em] text-foreground/40 ml-1">Client Identity</label>
                <input
                  type="text"
                  required
                  value={formData.customerName}
                  onChange={(e) => setFormData({ ...formData, customerName: e.target.value })}
                  className="w-full px-5 py-4 bg-white/5 border border-white/10 rounded-2xl text-foreground focus:outline-none focus:ring-2 focus:ring-amber-500/40 focus:bg-white/10 transition-all font-bold text-base"
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-black uppercase tracking-[0.2em] text-foreground/40 ml-1">Signal Channel (Email)</label>
                <input
                  type="email"
                  value={formData.customerEmail}
                  onChange={(e) => setFormData({ ...formData, customerEmail: e.target.value })}
                  className="w-full px-5 py-4 bg-white/5 border border-white/10 rounded-2xl text-foreground focus:outline-none focus:ring-2 focus:ring-amber-500/40 transition-all font-medium"
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-black uppercase tracking-[0.2em] text-foreground/40 ml-1">Amplitude (Rating)</label>
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/10 shadow-inner">
                  ...
                  />
                  <div className="flex items-center gap-1.5 bg-amber-500/20 px-3 py-1 rounded-full border border-amber-500/30">
                    <Star size={14} className="text-amber-500 fill-amber-500" />
                    <span className="font-black text-amber-500 text-lg">{formData.rating}</span>
                  </div>
                </div>
              </div>

              <div className="space-y-1.5">
                <label className="text-xs font-black uppercase tracking-[0.2em] text-foreground/40 ml-1">The Message</label>
                <textarea
                  required
              ...
              <div className="flex items-center gap-4 py-2 px-1">
                ...
                >
                  ...
                  <span className={`block h-8 w-14 rounded-full border-2 border-white/10 transition-colors shadow-inner ${formData.displayOnLanding ? 'bg-amber-500/80' : 'bg-white/10'}`} />
                  <span className={`absolute left-1 top-1 h-6 w-6 rounded-full bg-white shadow-xl transition-transform duration-300 ${formData.displayOnLanding ? 'translate-x-6' : 'translate-x-0'}`} />
                </div>
                <label className="text-sm font-black text-foreground/70 cursor-pointer select-none uppercase tracking-widest" onClick={() => setFormData({ ...formData, displayOnLanding: !formData.displayOnLanding })}>
                  Broadcast Live
                </label>
              </div>
        ...
        <div className="xl:col-span-8 space-y-6">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-6 px-2">
            <div className="relative w-full sm:w-[450px] group">
              ...
              <input 
                ...
                className="w-full pl-14 pr-6 h-16 bg-white/5 border border-white/10 rounded-3xl focus:outline-none focus:ring-4 focus:ring-amber-500/20 font-bold text-base transition-all shadow-inner"
                value={searchQuery}
          ...
          <div className="glass overflow-hidden rounded-[3rem] border-white/10 shadow-6xl relative">
                    ...
                    <th className="p-8 w-20">
                      <button onClick={toggleSelectAll} className="text-foreground transition-all cursor-pointer hover:scale-125 active:scale-90">
                        {selectedIds.length === filteredReviews.length && filteredReviews.length > 0 ? (
                          <div className="p-2 bg-amber-500 rounded-xl shadow-[0_4px_12px_rgba(245,158,11,0.4)]"><CheckSquare className="w-5 h-5 text-white" /></div>
                        ) : (
                          <div className="p-2 bg-white/10 rounded-xl hover:bg-white/20 shadow-inner"><Square className="w-5 h-5" /></div>
                        )}
                      </button>
                    </th>
                    <th className="p-8 font-black text-[10px] uppercase tracking-[0.3em] text-foreground/30">The Client</th>
                    <th className="p-8 font-black text-[10px] uppercase tracking-[0.3em] text-foreground/30">The Sentiment</th>
                    <th className="p-8 font-black text-[10px] uppercase tracking-[0.3em] text-foreground/30 text-center">Presence</th>
                     ...
                     <tr>
                        <td colSpan={5} className="p-32 text-center">
                           <div className="flex flex-col items-center gap-4">
                              ...
                              <p className="font-black text-xs uppercase tracking-widest text-amber-500/60">Scanning Chronicles...</p>
                           </div>
                        </td>
                     </tr>
                    ...
                    <tr>
                      <td colSpan={5} className="p-32 text-center">
                        <div className="flex flex-col items-center gap-6 opacity-20 grayscale">
                           ...
                           <p className="text-3xl font-black uppercase tracking-[0.5em]">No Resonance</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    filteredReviews.map((review) => (
                      <tr key={review.id} className={`group hover:bg-white/5 transition-all duration-500 ${selectedIds.includes(review.id) ? 'bg-amber-500/5' : ''}`}>
                        <td className="p-8">
                          <button onClick={() => toggleSelectOne(review.id)} className="text-foreground transition-all cursor-pointer hover:scale-125 active:scale-90">
                            {selectedIds.includes(review.id) ? (
                              <div className="p-2 bg-amber-500 rounded-xl shadow-lg ring-4 ring-amber-500/10"><CheckSquare className="w-5 h-5 text-white" /></div>
                            ) : (
                              <div className="p-2 bg-white/5 rounded-xl group-hover:bg-white/10 transition-colors shadow-inner"><Square className="w-5 h-5" /></div>
                            )}
                          </button>
                        </td>
                        <td className="p-8">
                          <div className="font-black text-xl text-foreground group-hover:text-amber-500 transition-colors tracking-tight uppercase italic">{review.customer_name}</div>
                        ...
                        <td className="p-8 max-w-md">
                           <div className="flex gap-1 mb-3">
                              {[...Array(5)].map((_, i) => (
                                <Star key={i} size={14} className={i < review.rating ? "text-amber-500 fill-amber-500" : "text-white/10"} />
                              ))}
                           </div>
                        ...
                        <td className="p-8 text-center">
                          <button
                            onClick={() => toggleDisplayOnLanding(review.id, review.display_on_landing)}
                          ...
                          >
                            {review.display_on_landing ? <Eye size={12} /> : <EyeOff size={12} />}
                            {review.display_on_landing ? "Public" : "Sequestered"}
                          </button>
                        </td>
                        <td className="p-8 text-right">
                          <div className="flex justify-end gap-3 translate-x-8 opacity-0 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-500">
                            <button
                              ...
                              className="w-12 h-12 flex items-center justify-center bg-white/5 hover:bg-amber-500 hover:text-white rounded-2xl text-foreground/50 transition-all cursor-pointer shadow-2xl ring-1 ring-white/10"
                            >
                              <Edit2 size={18} />
                            </button>
                            <button
        ...
        <AlertDialogContent className="glass border-white/20 shadow-4xl max-w-xl p-12 rounded-[4rem] overflow-hidden">
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-center text-foreground/60 text-xl py-6 leading-relaxed font-semibold">
              {isBulkDelete ? (
                <span>You are about to initiate an irreversible memory wipe of <strong>{deleteName}</strong>. This action will permanently sever these sentiments from the database.</span>
              ) : (
                <div className="space-y-6">
                   <p>Do you wish to permanently silence the echo from <strong>{deleteName}</strong>?</p>
                   <div className="bg-black/30 p-8 rounded-[3rem] border border-white/10 space-y-3 text-left shadow-2xl">
                      <div className="flex items-center gap-3 text-[11px] font-black text-amber-500 uppercase tracking-[0.3em]"><Info size={16}/> Hardware Protocol ID</div>
                      <code className="text-sm text-white font-mono break-all font-bold block bg-white/5 p-4 rounded-2xl border border-white/5">{deleteId}</code>
                   </div>
                </div>
              )}
            </AlertDialogDescription>
