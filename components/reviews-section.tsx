...
export default function ReviewsSection() {
  ...
  const validateForm = () => {
    ...
    if (!formData.reviewText.trim())
      newErrors.reviewText = "Please write a few words about your experience";
        ...
        <div className="glass p-8 space-y-6">
          <div>
            ...
            <p className="text-foreground/70">
              Tell us about your hand casting experience. Your review helps
              other families discover CastKeep!
            </p>
          </div>

          {submitted ? (
            <div className="bg-primary/20 border border-primary/50 rounded-lg p-4 text-primary">
              Thank you for your review! The admin will review and publish it
              shortly.
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  {errors.customerName && (
                    <p className="text-[10px] font-black uppercase text-destructive ml-1 mt-1">
                      {errors.customerName}
                    </p>
                  )}
                </div>
                  ...
                  {[1, 2, 3, 4, 5].map((num) => (
                    <button
                      ...
                      className={`transition-all ${
                        num <= formData.rating ? "scale-110" : "opacity-60"
                      }`}
                    >
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full px-4 py-2 bg-primary text-foreground font-medium rounded-lg hover:shadow-lg transition-all disabled:opacity-50"
              >
