...
export default function CustomerVisitsTable() {
  ...
  const [sorting, setSorting] = useState<SortingState>([{ id: "visit_timestamp", desc: true }])
  ...
  const fetchVisits = async () => {
    ...
    try {
      ...
      let query = supabase.from("customer_visits").select("*").order("visit_timestamp", { ascending: false })
  ...
  const columns: ColumnDef<CustomerVisit>[] = [
    ...
    },
    {
      ...
      header: () => <div className="uppercase tracking-[0.2em] text-[10px] font-black">Entry Point</div>,
      cell: ({ row }) => (
    ...
    },
    {
      ...
      header: () => <div className="uppercase tracking-[0.2em] text-[10px] font-black">IP Architecture</div>,
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
            <Globe size={12} className="text-amber-500/30" />
            <span className="text-xs font-mono font-black text-foreground/40">{row.getValue("ip_address") || "0.0.0.0"}</span>
        </div>
      ),
    },
    {
      accessorKey: "device_info",
      header: () => <div className="uppercase tracking-[0.2em] text-[10px] font-black">Hardware Protocol</div>,
      cell: ({ row }) => (
        <div className="flex items-center gap-4">
          <div className="p-2 bg-white/5 rounded-xl border border-white/10">
             {row.original.device_type === "mobile" ? <Monitor size={14} className="text-amber-500" /> : <Monitor size={14} className="text-amber-500" />}
          </div>
          <div>
            <div className="text-xs font-black text-foreground/70 uppercase tracking-tight italic">{row.original.browser}</div>
            <div className="text-[10px] font-bold text-foreground/30 uppercase tracking-widest mt-0.5">{row.original.os} • {row.original.device_type}</div>
          </div>
        </div>
      ),
    },
      ...
      <div className="flex justify-start">
        <Select value={timeFilter} onValueChange={setTimeFilter}>
          ...
          <SelectContent className="glass border-white/10 rounded-2xl p-2">
            <SelectItem value="all" className="rounded-xl font-black uppercase tracking-widest text-[10px] py-3">All Time Records</SelectItem>
            <SelectItem value="today" className="rounded-xl font-black uppercase tracking-widest text-[10px] py-3">Today Only</SelectItem>
            <SelectItem value="month" className="rounded-xl font-black uppercase tracking-widest text-[10px] py-3">Monthly Span</SelectItem>
            <SelectItem value="quarter" className="rounded-xl font-black uppercase tracking-widest text-[10px] py-3">Quarterly Span</SelectItem>
            <SelectItem value="year" className="rounded-xl font-black uppercase tracking-widest text-[10px] py-3">Annual Cycle</SelectItem>
          </SelectContent>
        </Select>
      </div>
               ...
               <TableRow>
                <td colSpan={columns.length} className="h-64 text-center">
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-[0.4em] text-amber-500 animate-pulse">Scanning Transmission Logs...</p>
                    </div>
                </td>
            </TableRow>
            ) : table.getRowModel().rows?.length ? (
              table.getRowModel().rows.map((row) => (
                <TableRow key={row.id} className="group border-white/5 hover:bg-white/[0.02] transition-colors">
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-64 text-center">
                   <div className="flex flex-col items-center gap-6 opacity-20 grayscale">
                        ...
                        <p className="text-2xl font-black uppercase tracking-[0.5em]">No Footprints Found</p>
                    </div>
                </TableCell>
              </TableRow>
            )}
