    ...
    <section id="gallery" className="py-20 px-4 md:px-8">
      <div className="max-w-6xl mx-auto">
        <div className="text-center space-y-4 mb-12">
          ...
          <p className="text-lg text-foreground/70">Inspiring hand casting projects from our customers</p>
        </div>
          ...
          {filtered.map((item) => (
            ...
            >
              ...
              <div className="p-4 space-y-2">
                ...
                <div className="flex items-center gap-1">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      size={14}
                      className={i < Math.floor(item.rating) ? "fill-accent text-accent" : "text-foreground/30"}
                    />
                  ))}
                ...
                {/* Customer Name */}
                {item.customer_name && <p className="text-xs font-medium text-foreground/80">{item.customer_name}</p>}

                {/* Customer Review */}
                {item.customer_review && (
                  <p className="text-xs text-foreground/60 line-clamp-2 italic">"{item.customer_review}"</p>
                )}
              </div>
            </div>
          ))}
        ...
        {filtered.length === 0 && (
          <div className="text-center py-12">
            <p className="text-foreground/60">No gallery items found in this category.</p>
          </div>
        )}
      </div>
    </section>
